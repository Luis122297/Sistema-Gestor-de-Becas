...
class ScholarshipController extends Controller
{
    ...
    public function recommend(Request $request, string $id): JsonResponse
    {
        ...
        if ($user->role !== 'profesor') {
            return response()->json(['message' => 'Acción exclusiva para profesores'], 403);
        }
        ...
        return response()->json([
            'message' => 'Recomendación guardada con éxito',
            'data'    => new ScholarshipResource($application),
        ]);
    }
    ...
    public function submitRequest(Request $request): JsonResponse
    {
        $user = $request->user();
        if ($user->role !== 'alumno') {
            return response()->json(['message' => 'Solo los alumnos pueden solicitar la beca.'], 403);
        }
    ...
    public function assignPercentage(Request $request, string $id): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'Porcentaje asignado correctamente',
            'data'    => new ScholarshipResource($application),
        ]);
    }
